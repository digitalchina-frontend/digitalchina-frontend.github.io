...
在贡献前，你需要了解 [前端网站目录结构规范](/docs/develop/vuejs/directory-structrue-specification/README.md)，以便于在正确的目录下创建文档文件并配置导航栏。
...
![emoji-log](/docs/.vuepress/public/images/contributing/emoji-log.png)

2. 选择一个 `commit type`，比如上传一篇文章，则选择 `📖 DOC: ` 类型

![commit-type](/docs/.vuepress/public/images/contributing/commit-type.png)

3. 进入 主侧边栏-源代码管理 补齐 `commit message` 的主题内容

![commit-message](/docs/.vuepress/public/images/contributing/commit-message.png)

4. 依次点击 `Commit`、`Push` 按钮

![commit-push](/docs/.vuepress/public/images/contributing/commit-push.png)

注意： 如果点击 Commit 按钮出现报错：`npx: command not found`，请尝试重启 VSCode，如仍然报错，建议通过命令行提交
